  ...
  return (
    <main className="mx-auto min-h-screen w-full max-w-[1100px] px-6 py-10">
      ...
      <p className="mt-2 text-sm text-muted">
        ...
        <code className="text-fg">src/components/ui.ts</code> (recipes) +{" "}
        <code className="text-fg">src/app/globals.css</code> (semantic tokens).
        This page is built only from them.
      </p>

      <Section
        title="Color — surfaces"
        hint="Backgrounds, low → high elevation."
      >
      ...
      <Section
        title="Radius"
        hint="By role: chips/icons → md · controls → lg · cards/modals → xl · columns → 2xl · status pills → full."
      >
        ...
        <div className="flex flex-col gap-2">
          {TYPE.map(([cls, use]) => (
            <div key={cls} className="flex items-baseline gap-4">
              <span className={`${cls} font-medium text-fg`}>
                The quick brown fox
              </span>
      ...
      <Section
        title="Motion"
        hint="Entrance animations + the looping scroll-hint nudge. prefers-reduced-motion neutralizes all."
      >
        <div className="flex flex-col gap-4">
          ...
          <div className="flex items-center gap-2 text-faint">
            <svg
              className="h-4 w-4 animate-hint-up"
              viewBox="0 0 20 20"
              fill="currentColor"
              aria-hidden="true"
            >
            ...
            <svg
              className="h-4 w-4 animate-hint-down"
              viewBox="0 0 20 20"
              fill="currentColor"
              aria-hidden="true"
            >
      ...
      <Section
        title="Buttons"
        hint="ui.ts btn.* — standard scale h-9 / px-3 / rounded-lg."
      >
      ...
      <Section
        title="Fields"
        hint="ui.ts field — one input/textarea/select base + canonical focus ring."
      >
      ...
      <Section
        ...
        hint="Active = raised neutral (bg-elevated / shadow-sm), never accent. ThemeToggle uses iconItem."
      >
      ...
      <Section
        ...
        hint="chip (data, rounded-md) vs pill (status/identity, rounded-full)."
      >
        <div className="flex flex-wrap items-center gap-3">
          <span className={chip}>
            <svg
          ...
          <span className={`${chip} bg-warning-subtle text-warning-text`}>
            due soon
          </span>
          <span className={`${chip} bg-danger-subtle text-danger-text`}>
            overdue
          </span>
      ...
      <Section title="Surfaces & components">
        <div className="flex flex-wrap items-start gap-4">
          ...
          <output className="inline-flex items-center gap-2 rounded-full border border-border bg-elevated/90 px-3 py-1.5 text-xs font-medium text-muted shadow-md">
      ...
      >
        <p className="text-sm text-muted">
          Scrim + blur, panel-in motion, pinned header/footer, stack-aware
          (ref-counted scroll lock, top-most Esc).
        </p>
      </Modal>

      <footer className="border-t border-border py-8 text-xs text-faint">
        leveling0 design system · edit recipes in src/components/ui.ts · tokens
