...
export function QuestModal({
  ...
  const [focusedField, setFocusedField] = useState<"note" | "detail" | null>(
    null,
  );
  ...
  const applyValues = useCallback((v: typeof baseValues) => {
    setTitle(v.title);
    setStatus(v.status);
    setDueAt(v.dueAt);
    setTags(v.tags);
  ...
  }, [open, draftKey, baseValues, applyValues]);
            ...
            <div className="flex flex-col gap-2.5 rounded-lg bg-warning-subtle px-3 py-2.5 text-warning-text sm:flex-row sm:items-center sm:justify-between">
              <div className="flex items-center gap-2">
                ...
                <div className="text-xs leading-snug">
                  ...
                  <p>
                    Recovered unsaved changes from {relativeAgo(restoredAt)}.
                  </p>
                </div>
              </div>
              <button
